...
const NotificationProvider = ({ 
  ...
  const [activeAttentionIds, setActiveAttentionIds] = useState<string[]>([]);

  const refreshData = useCallback(async () => {
    const today = dateUtils.getKyivToday();
    
    // Logic: Deadline is today or in the past AND status is not finished
    let query = supabase
    ...
    if (!error && data) {
      ...
      setActiveAttentionIds(ids);

      // Compare with IDs the user has already "seen" to get the badge count
  ...
  const markAsSeen = useCallback(() => {
    const seenIds: string[] = JSON.parse(localStorage.getItem('seen_doc_ids') || '[]');
    // Keep only the most recent 200 to keep localStorage clean
    const updated = Array.from(new Set([...seenIds, ...activeAttentionIds])).slice(-200);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(refreshData, 3600000); // Refresh every hour
