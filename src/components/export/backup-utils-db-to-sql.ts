...
//   try {
//     const timestamp = new Date().toISOString();
//     let sql = `-- MOTO CRM DATA DUMP WITH EMISSION AUTH LOGIC\n-- Generated: ${timestamp}\n\n`;
...
//       data.forEach(row => {
//         // SPECIAL CASE: Profiles handling raw target SQL injection block
//         if (table === 'profiles' && row.auth_user_id) {
//           const email = row.last_synced_email || row.email;
          
//           if (email) {
//             const escapedEmail = email.replace(/'/g, "''");
//             const escapedPhone = row.phone ? `'${row.phone.replace(/'/g, "''")}'` : 'NULL';
//             const fullName = `${row.first_name || ''} ${row.last_name || ''}`.trim().replace(/'/g, "''");
            
//             sql += `DO $$\n`;
//             sql += `DECLARE\n`;
//             sql += `  new_uid uuid := gen_random_uuid();\n`;
//             sql += `BEGIN\n`;
//             // 1. Create auth user row if the email doesn't exist on target yet
//             sql += `  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = '${escapedEmail}') THEN\n`;
//             sql += `    INSERT INTO auth.users (id, instance_id, email, phone, confirmed_at, raw_app_meta_data, raw_user_meta_data, created_at, updated_at, aud, role, is_super_admin)\n`;
//             sql += `    VALUES (new_uid, '00000000-0000-0000-0000-000000000000', '${escapedEmail}', ${escapedPhone}, now(), '{"provider":"email","providers":["email"]}', '{"full_name":"${fullName}"}', now(), now(), 'authenticated', 'authenticated', false);\n`;
//             sql += `  ELSE\n`;
//             sql += `    SELECT id INTO new_uid FROM auth.users WHERE email = '${escapedEmail}';\n`;
//             sql += `  END IF;\n\n`;
            
//             // 2. Insert profile using the matched or newly created auth user ID
//             const values = cols.map(c => {
//               if (c === 'auth_user_id') return 'new_uid'; // Inject variable reference instead of raw string
...
//             sql += `  INSERT INTO "public"."profiles" ("${cols.join('", "')}") VALUES (${values.join(', ')});\n`;
//             sql += `END $$;\n`;
//             return; // Skip normal standard single INSERT processing loop execution for this item
//           }
//         }

//         // STANDARD PROCESSING: For non-auth profiles and all other dataset layers
//         const values = cols.map(c => {
//           const val = row[c];
//           if (val === null) return 'NULL';
//           if (typeof val === 'boolean') return val ? 'true' : 'false';
