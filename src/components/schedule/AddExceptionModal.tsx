...
export function AddExceptionModal({ isOpen, onClose, instructorId, onSuccess, initialDate, editException }: Props) {
  ...
  return (
    <BaseModal
      isOpen={isOpen}
      onClose={onClose}
      title={editException ? 'Редагувати блокування' : 'Блокування часу'}
      footer={
    ...
    >
      <form className="space-y-5">
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="pb-3 text-slate-600"><ArrowRight size={16} /></div>
          <div className="flex-1 space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-500 ml-1 flex items-center gap-1">Кінець</label>
            <input 
              type="date"
              value={endDate}
              min={startDate}
        ...
        {!isAllDay && (
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <input type="time" step="600" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-primary [color-scheme:dark]" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-500 ml-1">Час кінця</label>
              <input type="time" step="600" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-primary [color-scheme:dark]" />
            </div>
          </div>
        )}
      </form>
