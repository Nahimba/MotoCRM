      ...
      <Form {...(form as any)}>
        <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-8">
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <Select onValueChange={field.onChange} value={field.value} disabled={isLocked}>
                    ...
                    <SelectContent className="z-[500] bg-[#0F0F0F] border-white/10 text-white">
                      {instructors.map((ins) => (
                        <SelectItem 
                          key={ins.id} 
                          value={ins.id} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className="bg-black border-white/5 h-16 text-white rounded-xl focus:ring-primary">
            ...
            {/* Course Field with "Довідник" button */}
            <FormField
              control={control}
              name="course_id"
              render={({ field }) => {
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <div className="flex items-center space-x-3 h-16 rounded-xl border border-white/5 px-4">
                    ...
                    <FormLabel className={`text-[10px] font-black uppercase flex items-center gap-2 cursor-pointer ${!isLocked ? "text-white" : "text-slate-600"}`}>
                      <Tag size={12} className="text-primary" /> Знижка
                    </FormLabel>
                  </div>
                </FormItem>
              )}
...
<BaseModal
  ...
  title="Довідник Курсів" // Передаем строку, чтобы соответствовать типу string
  className="z-[500]"
  icon={<BookOpen size={20} className="text-primary" />} // Можно добавить иконку, если нужно
>
  <div className="p-6 md:p-8 space-y-6">
    {/* CUSTOM STYLED TITLE (так как пропс title только для string) */}
    {/* <div className="mb-2">
    ...
    <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-3 no-scrollbar">
      {courses.map((c) => {
        ...
        return (
          <div 
            key={c.id} 
            className="group border border-white/5 bg-white/[0.02] p-5 rounded-[1.8rem] transition-all hover:bg-white/[0.04] hover:border-primary/20 hover:border-white/10"
          >
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col items-end justify-center">
                  <div className="bg-primary px-4 py-2 rounded-xl shadow-lg shadow-primary/20">
