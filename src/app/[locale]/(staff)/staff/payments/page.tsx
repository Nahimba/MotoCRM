            ...
            <tbody className="divide-y divide-white/[0.03]">
              {loading ? <LoadingRow /> : filteredPayments.map((p) => (
                <tr key={p.payment_id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <span className="font-black text-base text-primary tabular-nums italic">{p.amount.toLocaleString()} UAH</span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-black uppercase text-white/70 italic">
          ...
          {loading ? (
            ...
            >
              <div className="flex flex-col">
                <p className="font-black text-xs uppercase italic leading-none mb-2 text-white">
                  {p.client_name} {p.client_last_name}
                </p>
              ...
              <div className="text-right flex flex-col items-end">
                <p className="font-black text-sm text-primary italic leading-none mb-2">{p.amount.toLocaleString()} UAH</p>
...
function StatusBadge({ status, t, mobile = false }: { status: string, t: any, mobile?: boolean }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border ${config.color} ${mobile ? 'text-[8px]' : 'text-[10px]'} font-black uppercase italic`}>
  ...
  return (
    <tr>
      <td colSpan={6} className="py-20 text-center">
        ...
        <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest italic">Syncing Ledger...</p>
      </td>
    </tr>
  )
}