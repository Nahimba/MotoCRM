    ...
    <div className="max-w-7xl mx-auto px-4 pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 py-6">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto items-center">
          ...
          <button 
            onClick={() => setShowOnlyMine(!showOnlyMine)}
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-3 py-3 rounded-xl border transition-all text-[9px] font-black uppercase tracking-widest ${
          ...
          <button 
            onClick={() => setShowOnlyActive(!showOnlyActive)}
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-3 py-3 rounded-xl border transition-all text-[9px] font-black uppercase tracking-widest ${
              showOnlyActive ? 'bg-white/10 border-white/20 text-white' : 'bg-red-500/10 border-red-500/50 text-red-500'
            }`}
          >
      ...
      <div className="relative mb-6">
        ...
        <input 
          ...
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full bg-white/5 border border-white/10 rounded-xl py-4 pl-12 pr-4 text-[11px] font-bold uppercase tracking-widest text-white outline-none focus:border-primary/50 transition-all"
        />
      </div>
      ...
      <div className="block md:hidden space-y-3">
        {loading ? (
           <div className="py-20 text-center bg-[#070707] rounded-2xl border border-white/5">
              <Bike className="mx-auto animate-bounce text-primary/20 mb-4" size={32} />
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Loading...</span>
           </div>
        ) : filteredClients.length === 0 ? (
          <div className="py-10 text-center text-slate-600 uppercase text-[10px] font-black tracking-widest">No pilots found</div>
        ) : filteredClients.map(client => {
          const hours = Number(client.total_remaining_hours) || 0;
          const debt = Number(client.total_debt) || 0;
          return (
            <Link href={`/staff/clients/${client.id}`} key={client.id} className="block bg-[#070707] border border-white/5 p-4 rounded-2xl active:scale-95 transition-all">
              <div className="flex justify-between items-start mb-3">
                <span className="font-black uppercase text-white italic tracking-tighter">{client.name} {client.last_name}</span>
                <span className={`text-xs font-black italic ${hours < 2 && showOnlyActive ? 'text-red-500' : 'text-slate-400'}`}>{hours}H</span>
              </div>
              <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg border text-[9px] font-black uppercase italic ${
                debt > 0 ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-green-500/10 border-green-500/20 text-green-500'
              }`}>
                <Wallet size={10} /> {debt > 0 ? `${Math.round(debt)} ₴` : t("cleared")}
              </div>
            </Link>
          )
        })}
      ...
      <div className="hidden md:block bg-[#070707] border border-white/5 rounded-2xl overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/5 bg-white/[0.02]">
              <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t("client")}</th>
          ...
          <tbody className="divide-y divide-white/5">
            {filteredClients.map(client => (
              <tr key={client.id} className="group hover:bg-white/[0.01] transition-colors">
                ...
                <td className="px-6 py-5 text-center">
                  <span className={`text-sm font-black italic ${!showOnlyActive ? 'text-zinc-600' : (Number(client.total_remaining_hours) < 2 ? 'text-red-500 animate-pulse' : 'text-white')}`}>
                    {Number(client.total_remaining_hours)}H
                  </span>
                </td>
                <td className="px-6 py-5 text-center">
                  <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border ${
                    Number(client.total_debt) > 0 ? 'bg-red-500/5 border-red-500/20 text-red-500' : 'bg-green-500/5 border-green-500/20 text-green-500'
                  }`}>
                    <Wallet size={12} />
                    <span className="text-[11px] font-black uppercase italic">
                      {Number(client.total_debt) > 0 ? `${Math.round(Number(client.total_debt))} ₴` : t("cleared")}
                    </span>
                  </div>
                </td>
              </tr>
