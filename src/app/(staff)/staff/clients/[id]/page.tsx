...
export default function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return (
    <div className="h-[80vh] flex flex-col items-center justify-center space-y-4">
      ...
      <div className="font-black animate-pulse text-primary uppercase tracking-[0.5em] text-xs">Accessing Dossier...</div>
    </div>
  )
  
  if (!client) return <div className="h-screen flex items-center justify-center text-white font-black uppercase">Rider Signal Lost</div>

  const account = client.accounts?.[0]
  const activePackage = account?.course_packages?.find((p: any) => p.status === 'active')
  
  // Debt calculation based on your payments table
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-6">
        <div className="space-y-4">
          <Link href="/staff/clients" className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors group">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Return to Fleet Roster</span>
          </Link>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-8 space-y-8 relative overflow-hidden">
            ...
            <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-white/5 pb-4">Core Intel</h2>
            <div className="space-y-6 relative z-10">
              <InfoRow icon={<Phone size={14}/>} label="Comms" value={client.phone} />
              <InfoRow icon={<Mail size={14}/>} label="Network" value={client.email} />
              <InfoRow icon={<MapPin size={14}/>} label="Sector" value={client.address} />
            </div>
            
            <div className="pt-4">
              <p className="text-[10px] font-black text-slate-600 uppercase mb-3 tracking-widest">Transmission Spec</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <StatCard 
              ...
              sub={`of ${activePackage?.total_hours || 0}H deployment`}
            />
            <StatCard 
              label="Financial Standing" 
              value={`${debt > 0 ? `-${debt}` : '0'} ₴`} 
              icon={<CreditCard size={28} className={debt > 0 ? "text-red-500" : "text-green-500"} />} 
              sub={debt > 0 ? "Outstanding Balance" : "Account Settled"}
            />
          </div>
          ...
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-10 relative overflow-hidden group">
              <div className="flex justify-between items-center mb-10">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-white">Active Training Module</h3>
              ...
              {activePackage ? (
                <div className="space-y-8">
                   <div className="flex justify-between items-end">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-2">Valuation: {activePackage.contract_price} ₴</p>
                      </div>
                      <div className="text-right">
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Completion</p>
                      </div>
                   </div>
                ...
                <div className="py-16 text-center border-2 border-dashed border-white/5 rounded-[2rem]">
                   <p className="text-slate-600 font-black uppercase tracking-[0.2em] text-xs">No Deployment Active</p>
                </div>
              )}
          </div>
        </div>
  ...
  return (
    <div className="flex items-center gap-5 group">
      ...
      <div>
        ...
        <p className="text-sm font-black text-white uppercase tracking-tight">{value || 'UNSPECIFIED'}</p>
      </div>
    </div>
  )
}
